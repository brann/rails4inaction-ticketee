...
RSpec.feature "Users can search for tickets matching specific criteira" do
  let(:user) { FactoryGirl.create(:user) }
  let(:project) { FactoryGirl.create(:project) }
  let(:open) { FactoryGirl.create(:state, name: "Open", default: true) }
  let(:closed) { FactoryGirl.create(:state, name: "Closed") }
  let!(:ticket_1) do
    FactoryGirl.create(:ticket, name: "Create projects", project: project,
                       author: user, tag_names: "iteration_1",
                       state: open)
  end
  let!(:ticket_2) do
    FactoryGirl.create(:ticket, name: "Create users", project: project,
                       author: user, tag_names: "iteration_2",
                       state: closed)
  end
